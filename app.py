...
# ==========================================
# 3. محرك السحب (صائد المقاسات والصور الصافي)
# ==========================================
def find_product_dict(data):
    """خوارزمية رادار تبحث عن صندوق بيانات المنتج في أي مكان داخل الأكواد"""
    if isinstance(data, dict):
        if 'images' in data and 'name' in data and ('allVariants' in data or 'variants' in data):
            return data
...
def fetch_trendyol_product(url):
    ...
    # 1. استخراج رقم المنتج
    sku_match = re.search(r'-p-(\d+)', clean_url)
    if sku_match: sku = sku_match.group(1)

    # 2. استخراج الجافا سكريبت المخفية
    json_data = None
    next_match = soup.find('script', id='__NEXT_DATA__')
    if next_match:
        try: json_data = json.loads(next_match.string)
        except: pass
    else:
    ...
    # 3. إطلاق الرادار للبحث عن بيانات المنتج بدقة
    ...
    if product_dict:
        ...
        # سحب الصور من المعرض فقط (يستحيل دخول شعار المركز السعودي هنا)
        for img in product_dict.get('images', []):
            ...
            if img_str:
                if not img_str.startswith('http'):
                    img_str = f"https://cdn.dsmcdn.com{img_str}"
                raw_images.append(img_str)

        # جدول المواصفات الإضافية
    ...
    # 4. خطة طوارئ (إذا فشلت الخطوة السابقة فقط)
    if not title:
    ...
    # هذا السطر يعمل فقط إذا كان المنتج فارغاً تماماً من الصور
    if not raw_images:
        html_imgs = re.findall(r'(https://cdn\.dsmcdn\.com/[^"\'\s<>]+?\.(?:jpg|jpeg|webp|png))', html, re.IGNORECASE)
        blacklist = ['logo', 'icon', 'flag', 'pci', 'iso', 'trust', 'badge', 'payment', 'footer', 'frontend', 'saudibusiness', 'sbc', 'stamp', 'rating', 'maroof', 'mada', 'visa']
        for img in html_imgs:
            if not any(bw in img.lower() for bw in blacklist):
                if 'productmedia' in img.lower() or '/ty/' in img.lower():
                    raw_images.append(img)

    # 5. تنقية الصور لأعلى دقة
    final_images = []
    ...
    if not title or price == 0.0:
        return {"error": "لم نتمكن من العثور على بيانات المنتج."}
...
if menu == "🚀 سحب منتج جديد":
    ...
    if st.button("🚀 جلب وتحليل بيانات المنتج", use_container_width=True):
        if not product_url:
            ...
            with st.spinner("جاري استخراج البيانات والمقاسات والصور الصافية..."):
